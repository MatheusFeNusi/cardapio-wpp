...
async function run() {
  try {
    // Querying PostgREST API to get OpenAPI schema or table details
    const r = await fetch(
      `${SB_URL}/rest/v1/`,
      { headers: SBH }
    );
    ...
    console.log('--- TABLE SCHEMA DEFINITIONS ---');
    console.log(JSON.stringify(data.definitions?.pedidos || {}, null, 2));
  } catch (e) {
